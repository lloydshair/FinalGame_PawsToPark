...
public class FinalHam1 : MonoBehaviour
{

    //animation 

    Animator anim;
    float dirX, moveSpeed4;
    ...
    void Update()
    {
        ...
        if (isPowerActive && moveSpeed4 < 4f)
        {
            moveSpeed4 = 4f;
        }
        else
        ...
        rbHamster.velocity = new Vector2(dirX * moveSpeed4, rbHamster.velocity.y);
        ...
        {
            rbHamster.velocity = new Vector2(-moveSpeed4, rbHamster.velocity.y);
        }
        ...
        if (Input.GetKey(up))
        {
            rbHamster.velocity = new Vector2(rbHamster.velocity.x, moveSpeed4);
        }
    ...
    {
        anim.SetBool("walking3", Mathf.Abs(dirX) > 0);
        anim.SetBool("running3", isPowerActive);
    }
    ...
    private void FixedUpdate()
    {
        if (input2 != Vector3.zero)
        {
            ...
            GetComponent<Rigidbody2D>().velocity = input2 * moveSpeed4; ;
    ...
    IEnumerator Move(Vector3 targetPos)
    {
        ...
        while ((targetPos - transform.position).sqrMagnitude > Mathf.Epsilon)
        {
            ...
            transform.position = Vector3.MoveTowards(transform.position, targetPos, moveSpeed4 * Time.deltaTime);
            yield return null;
        }
    ...
    public void ActivatePowerup()
    {
        if (!isPowerActive)
        {
            ...
            Debug.Log("Powerup activated. New moveSpeed: " + moveSpeed4);
        }
        else
        {
            powerTimer = powerDuration;
    ...
    private void DeactivatePowerup()
    {
        moveSpeed4 -= 3f;
        isPowerActive = false;
        Debug.Log("Powerup gone speed gone back to " + moveSpeed4);
    }
