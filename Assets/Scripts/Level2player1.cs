...
public class Level2player1 : MonoBehaviour
{

    //animation 

    Animator anim;
    float dirX, moveSpeed3;
    ...
    void Update()
    {
        ...
        if (isPowerActive && moveSpeed3 < 4f)
        {
            moveSpeed3 = 4f;
        }
        else
        ...
        rbHamster.velocity = new Vector2(dirX * moveSpeed3, rbHamster.velocity.y);
        ...
        {
            rbHamster.velocity = new Vector2(-moveSpeed3, rbHamster.velocity.y);
        }
        ...
        if (Input.GetKey(up))
        {
            rbHamster.velocity = new Vector2(rbHamster.velocity.x, moveSpeed3);
        }
    ...
    private void FixedUpdate()
    {
        if (input2 != Vector3.zero)
        {
            ...
            GetComponent<Rigidbody2D>().velocity = input2 * moveSpeed3; ;
    ...
    IEnumerator Move(Vector3 targetPos)
    {
        ...
        while ((targetPos - transform.position).sqrMagnitude > Mathf.Epsilon)
        {
            ...
            transform.position = Vector3.MoveTowards(transform.position, targetPos, moveSpeed3 * Time.deltaTime);
            yield return null;
        }
    ...
    public void ActivatePowerup()
    {
        if (!isPowerActive)
        {
            ...
            Debug.Log("Powerup activated. New moveSpeed: " + moveSpeed3);
        }
        else
        {
            powerTimer = powerDuration;
    ...
    private void DeactivatePowerup()
    {
        moveSpeed3 -= 3f;
        isPowerActive = false;
        Debug.Log("Powerup gone speed gone back to " + moveSpeed3);
    }
