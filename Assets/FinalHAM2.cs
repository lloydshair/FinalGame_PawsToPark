...
public class FinalHAM2 : MonoBehaviour
{

    //animation 

    Animator anim;
    float dirX, moveSpeed5;
    ...
    void Update()
    {
        ...
        if (isPowerActive && moveSpeed5 < 4f)
        {
            moveSpeed5 = 4f;
        }
        else
        ...
        rbHamster.velocity = new Vector2(dirX * moveSpeed5, rbHamster.velocity.y);
        ...
        {
            rbHamster.velocity = new Vector2(-moveSpeed5, rbHamster.velocity.y);
        }
        ...
        if (Input.GetKey(up))
        {
            rbHamster.velocity = new Vector2(rbHamster.velocity.x, moveSpeed5);
        }
    ...
    private void FixedUpdate()
    {
        if (input2 != Vector3.zero)
        {
            ...
            GetComponent<Rigidbody2D>().velocity = input2 * moveSpeed5; ;
    ...
    IEnumerator Move(Vector3 targetPos)
    {
        ...
        while ((targetPos - transform.position).sqrMagnitude > Mathf.Epsilon)
        {
            ...
            transform.position = Vector3.MoveTowards(transform.position, targetPos, moveSpeed5 * Time.deltaTime);
            yield return null;
        }
    ...
    public void ActivatePowerup()
    {
        if (!isPowerActive)
        {
            ...
            Debug.Log("Powerup activated. New moveSpeed: " + moveSpeed5);
        }
        else
        {
            powerTimer = powerDuration;
    ...
    private void DeactivatePowerup()
    {
        moveSpeed5 -= 3f;
        isPowerActive = false;
        Debug.Log("Powerup gone speed gone back to " + moveSpeed5);
    }
